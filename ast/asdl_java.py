...
def get_java_type(name):
    """Return a string for the Java name of the type.
...
def reflow_java_string(s, depth):
    return '"%s"' % s.replace('\n', '\\n" +\n%s"' % (' ' * depth * TABSIZE))
...
class EmitVisitor(asdl.VisitorBase):
    ...
    def __init__(self, target_dir, metadata = None):
        self.target_dir = target_dir
        self._metadata = metadata
        super(EmitVisitor, self).__init__()

    def open(self, name, refersToSimpleNode=1, useDataOutput=0):
        self.file = open(Path(self.target_dir, "%s.java" % name), "w")
        print("// Autogenerated AST node", file=self.file) 
        print('package org.python.parser.ast;', file=self.file) 
        if refersToSimpleNode:
            print('import org.python.parser.SimpleNode;', file=self.file) 
        if useDataOutput:
...
class JavaVisitor(EmitVisitor):
    ...
    def javaMethods(self, type, clsname, ctorname, fields, depth):
        ...
        for f in fields:
            ...
            if str(f.type) not in self.bltinnames and f.typedef and f.typedef.simple:
...
class VisitorVisitor(EmitVisitor):
    def __init__(self, target_dir):
        EmitVisitor.__init__(self, target_dir)
        self.ctors = []
...
if __name__ == "__main__":
    ...
    main(args.input_file, args.target, args.dump_module)
